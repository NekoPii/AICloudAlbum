...
class SignupForm(forms.Form):
    phone = forms.CharField(min_length=11, max_length=11,
                            widget=forms.TextInput(
                                attrs={"class": "form-control-input", "id": "signup_phone", "tabindex": "1","autocomplete":"off"}))
    name = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control-input", "id": "signup_name", "tabindex": "2","autocomplete":"off"}))
    pwd = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control-input", "id": "signup_pwd", "tabindex": "3","autocomplete":"off"}))
    re_pwd = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control-input", "id": "re_signup_pwd", "tabindex": "4","autocomplete":"off"}))
    def clean_phone(self):
        phone=self.cleaned_data["phone"]
        pattern=re.compile(r"^(1[3-9][0-9]{9})")
        if pattern.match(phone):
